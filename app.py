...
st.title("ASK QUESTION FROM THE DOCUMENT")
...
def vector_embedding():
    if "vectors" not in st.session_state:
        ...
        st.session_state.loader = PyPDFDirectoryLoader("./data_corpus")  ## Data Ingestion
        ...
        st.session_state.vectors = FAISS.from_documents(st.session_state.final_documents,
                                                        st.session_state.embeddings)  #vector OpenAI embeddings


prompt1 = st.text_input("Enter Your Question From Documents")

if st.button("Create Documents Embedding"):
    vector_embedding()
